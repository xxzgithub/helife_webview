...
import com.hilife.webview.model.js.JSStartCustomServerParam;
...
import cn.net.cyberwy.hopson.lib_util.CalculationUtil;
...
public abstract class DJJavascriptImpl implements DJJavaScript {
    ...
    public DJJavascriptImpl(BaseActivity context, WebView webView) {
        super();
        ...
        funSet.add("getFile");
        funSet.add("getTopic");
        funSet.add("addComment");
        funSet.add("ajaxProxy");
        ...
        funSet.add("chooseCommunity");
        funSet.add("closeWindowAndChangeCompany");
        funSet.add("getDeviceInfo");
        funSet.add("showFeedDetail");
        funSet.add("umBridge");
        funSet.add("hilifepay");
    ...
    // TODO: 2020/5/21 切换小区
    public void chooseCommunityIn(JSFeedParam feed) {
        com.orhanobut.logger.Logger.t("注册流程").i("chooseCommunityIn " + feed.toString());
...
//		mApplication.exitToMainActivity(mContext, data);
//		mContext.overridePendingTransition(R.anim.hold, R.anim.slide_down_out);
        if (feed.getType() == 3) {//app界面切换小区
            DJCacheUtil.keepCommunityID(feed.getCompanyID());
            DJCacheUtil.keep(DJCacheUtil.COMMUNITY_NAME, feed.getCompanyName());
            DJCacheUtil.keep(DJCacheUtil.COMMUNITY_SHORTCHAIN, Configuration.getWebShortChain(mContext) + feed.getShortURLID());
            ((GlobalApplication) GlobalApplication.getContext()).exitToMainActivity(mContext);
            mContext.overridePendingTransition(R.anim.hold, R.anim.slide_down_out);
            VisitorCommunityUtil.doAddThisCommunity(mContext, ((WebModuleAppLifecycleImpl) WebModuleAppLifecycleImpl.getContext().getContext()), DJCacheUtil.readCommunityID());
        } else if (feed.getType() == 4) {
            DJCacheUtil.keepCommunityID(feed.getCompanyID());
            DJCacheUtil.keep(DJCacheUtil.COMMUNITY_NAME, feed.getCompanyName());
            ((GlobalApplication) GlobalApplication.getContext()).exitToMainActivity(mContext);
            mContext.overridePendingTransition(R.anim.hold, R.anim.slide_down_out);
        } else if (feed.getType() == 5) {
            Intent data = new Intent();
            data.putExtra("username", feed.getMobile());
            data.putExtra("password", feed.getPassword());
            data.putExtra("community", DJCacheUtil.readCommunityID());
            mContext.setResult(Constants.WEB_REGISTER, data);
            mContext.finish();
            mContext.overridePendingTransition(0, R.anim.slide_down_out);
        } else {
            Intent data = new Intent();
            data.putExtra("username", feed.getMobile());
            data.putExtra("password", feed.getPassword());
            data.putExtra("community", feed.getCompanyID());
            data.putExtra("communityName", feed.getCompanyName());
//		data.putExtra("cStatus", cStatus);
    ...
    @Override
    @JavascriptInterface
    public final void chooseCommunity(String param) {
        Logger.D(TAG, "jsChooseCommunity:" + param);
        JSFeedParam feed = parse(param, JSFeedParam.class);
        if (feed != null) {
            chooseCommunityIn(feed);
        }
    }
    ...
    @Override
    @JavascriptInterface
    public final void notifyPasswordSuccess(String username) {
        DialogUtil.showAlert(mContext,
                ...
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                        NormalUtils.doLogout(mContext, (GlobalApplication) mContext.getApplication(), false);
                    }
                }, false).show();
    }
...
//            mContext.startActivity(intent);
    ...
    public void showGroupSearchIn() {
        // TODO: 4/17/21  showGroupSearchIn
//        IntentUtil.openIntent(mContext, GroupSearchActivity.class);
    }
                ...
                public void run() {
                    ...
                    if (StringUtil.isNotBlank(paramObj.getIsShare())) {
                        if (new Boolean(paramObj.getIsShare()) || "1".equals(paramObj.getIsShare())) {
                            ...
                            initShareOptMenuIn(optMenuParam, JSONUtil.toJSON(optMenuParam));
                        } else {
    ...
    public void getLocation(String param) {
        Logger.D(TAG, "civ_steward_icon:" + param);
        final JSGetLocationParam paramObj = parse(param, JSGetLocationParam.class);
        if (paramObj != null) {
    ...
    public void getDeviceInfoIn(final JSGetLocationParam paramObj) {
        if (!PermissionUtil.verifyLocationPermissions(mContext)) {
            return; //没有定位权限就不能执行逻辑
        }
        if (paramObj != null) {
...
//							location.put("lat", mNativeLocation.getLat());
//							location.put("city", mNativeLocation.getCityName());
                        //LBS信息，格式：维度/经度（正北纬、东经，精确两位）正负号
                        String latStr, lngStr;
                        if (Double.valueOf(mNativeLocation.getLat()) > 0) {
                            latStr = "+" + CalculationUtil.getCommonNumber(mNativeLocation.getLat(), 2);
                        } else {
                            latStr = mNativeLocation.getLat();
                        }
                        if (Double.valueOf(mNativeLocation.getLat()) > 0) {
                            lngStr = "+" + CalculationUtil.getCommonNumber(mNativeLocation.getLng(), 2);
                        } else {
                            lngStr = mNativeLocation.getLng();
                        }
                        location.put("lbs", latStr + "/" + lngStr);
                        //设备标识（IMEI）
                        location.put("deviceID", NetworkUtil.getDeviceId(mContext));
                        //设备SIM卡号（国家代码+手机号，多个用英文逗号隔开）
                        if (StringUtil.isEmpty(NetworkUtil.getPhoneNum(mContext))) {
                            location.put("fullDeviceNumber", "");
                        } else {
                            location.put("fullDeviceNumber", "+" + NetworkUtil.getPhoneCountryCode(mContext) + NetworkUtil.getPhoneNum(mContext));
                        }
                        //01：手机APP
                        location.put("appName", "01 合生活");
                        //SIM卡数量1:一张，2：两张，3：其他
                        location.put("simCardCount", NetworkUtil.getPhoneCount(mContext));
                        //设备类型1：手机
                        location.put("deviceType", NetworkUtil.isTablet(mContext) ? "2" : "1");
                        //设备型号名称
                        location.put("deviceName", Build.MODEL);
                        //MAC地址每两位用-链接
                        String macAddr = NetworkUtil.getMacAddress(mContext).replaceAll(":", "-");
                        location.put("macAddr", macAddr);
                        //IP版本号04:IPV4,06:IPV6
//							location.put("ipType", "");
                        //IP地址IPV4:点分十进制，IPV6:冒号分十六进制
                        location.put("sourceIP", NetworkUtil.getLocalIpAddress());
                        param = JSReturnParam.success(paramObj.getCallbackId(), location);
                    } else {
                        param = JSReturnParam.fail(paramObj.getCallbackId(), "");
                    }
                ...
                public void onLocationError() {
                    JSReturnParam param;
                    Map location = new HashMap();
                    //LBS信息，格式：维度/经度（正北纬、东经，精确两位）
                    location.put("lbs", "0/0");
                    //设备标识（IMEI）
                    location.put("deviceID", NetworkUtil.getDeviceId(mContext));
                    //设备SIM卡号（国家代码+手机号，多个用英文逗号隔开）
                    if (StringUtil.isEmpty(NetworkUtil.getPhoneNum(mContext))) {
                    ...
                    //MAC地址每两位用-链接
                    String macAddr = NetworkUtil.getMacAddress(mContext).replaceAll(":", "-");
                    location.put("macAddr", NetworkUtil.getMacAddress(mContext));
                    //IP版本号04:IPV4,06:IPV6
//							location.put("ipType", "");
                    //IP地址IPV4:点分十进制，IPV6:冒号分十六进制
                    location.put("sourceIP", NetworkUtil.getLocalIpAddress());
...
//        }
    ...
    /**
     * @param optMenuParam
     * @param param        新增js返回原始数据，由于JSShowOptMenuParam与ShareBean
     */
    ...
    public void showOptMenu(String param) {
        ...
        if (optMenuParam != null) {
            showOptMenuIn(optMenuParam, param);
        }
    }
    ...
    public void initShareOptMenu(String param) {
        ...
        Timber.i("initShareOptMenummm: " + param);
        JSShowOptMenuParam optMenuParam = parse(param, JSShowOptMenuParam.class);
        if (optMenuParam != null) {
            Timber.i("不为空: ");
            initShareOptMenuIn(optMenuParam, param);
        } else {
            Timber.i("为空: ");
        }
    }
    ...
    public void startCustomServer(String param) {
        Logger.D(TAG, "startCustomServer:" + param);
        JSStartCustomServerParam imConversationParam = parse(param, JSStartCustomServerParam.class);
        if (imConversationParam != null) {
            startCustomServerIn(imConversationParam);
        }
    }
        ...
        new AsyncTask<Void, Void, List<Map<String, Object>>>() {
            ...
            protected List<Map<String, Object>> doInBackground(Void... params) {
                if (Integer.valueOf(1).equals(uploadFileByAppParam.getIsShowProgress())) {
                    DJJavascriptImpl.this.handler.post(new Runnable() {
                        @Override
                        public void run() {
            ...
            protected void onPostExecute(List<Map<String, Object>> result) {
                ...
                Message msg = handler.obtainMessage(9);
                msg.obj = param;
                DJJavascriptImpl.this.handler.sendMessage(msg);
    ...
    public void getWebHost(String param) {
        ...
        Message msg = new Message();
        msg.what = 9;
        msg.obj = JSReturnParam.success(jsParam.getCallbackId(), Configuration.getWebHost(mContext));
        handler.sendMessage(msg);
    }
    ...
    public void ajaxProxy(String param) {
        ...
        if (null != paraMap) {
            if (paraMap.containsKey("url")) {
                ...
                Log.v("ajaxProxy", "start: " + (System.currentTimeMillis() - start));
                Log.v("ajaxProxy", "url = " + url + ",json = " + JSONUtil.toJSON(mapForRequest));
                ServiceFactory.getH5TemplateService(mContext).ajaxProxy(Configuration.getWebHost(mContext) + url, methodType, mapForRequest, new DataCallbackHandler<Void, Void, String>() {
                    @Override
                    public void onSuccess(String s) {
                        Log.v("ajaxProxy", "end: " + (System.currentTimeMillis() - start));
                        Message msg = new Message();
                        msg.what = 9;
                        try {
...
//						super.onError(e);
    ...
    public void setNavigationTitle(String param) {
        Logger.D(TAG, "setNavigationTitle");
    }
    ...
    public void myCustomerServiceInfo() {
        // TODO: 4/17/21  myCustomerServiceInfo
        // IM客服
...
//                .appendQueryParameter("targetId", ConnectRongUtils.getCustomerServiceID()).appendQueryParameter("title", "")
//                .build();
...
//        mContext.startActivity(intent);
    ...
    public void openFlutterPage(String param) {
        ...
        Logger.D(TAG, "openFlutterPage---->" + param);
    }
     ...
     * js调用端的获取状态栏高度
     *
     * @param params js调用返回结果
     */
    @JavascriptInterface
    @Override
    public void getStatusHeight(String params) {
        Logger.E("getStatusHeight", params);
        if (!TextUtils.isEmpty(params)) {
            final Map paraMap = JSONUtil.parseJSON(params, Map.class);
            final String callbackId = (String) paraMap.get("callbackId");
            Message msg = Message.obtain();
            msg.what = 9;
            if (null != paraMap) {
                Map<String, String> paramsMap = new HashMap<>();
                paramsMap.put("statusHeight", (((DensityUtil.dip2px(mContext, ScreenUtil.getStatusBarHeight(mContext) / 2))) / 2) / 2 + "");
                paramsMap.put("fromSource", "android");
                msg.obj = JSReturnParam.success(callbackId, paramsMap);
            } else {
                msg.obj = JSReturnParam.fail(callbackId, "");
            }
            handler.sendMessage(msg);
